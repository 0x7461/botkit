...
Both use `snooze` + runit. RSS bot deduplicates via SQLite so it's safe to run frequently.
...
- Multiple bots share the same `TELEGRAM_BOT_TOKEN` but run independently
